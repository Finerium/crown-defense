...
| Latensi containment ujung ke ujung di produksi | 1 sampai 13 ms `containment_wall_ms`, diukur ulang pada 4 Agustus 2026 atas 12 run tersimpan yang membawa angka (1, 1, 3, 3, 3, 3, 3, 3, 4, 6, 11, 13); deteksi 1 sampai 11 ms. Ini termasuk penyegelan catatan audit dan penerbitan perintah, jadi ia **bukan** angka 0,066 ms yang ada di baris atas: yang itu jalur keputusan kebijakan saja dengan sink audit di-spy | catatan permanen di `GET /api/insiden`, bisa dibuka siapa pun |
...
| Containment p95 di bawah 10 detik | p95 0,066 ms pada 200 run untuk keputusan kebijakan saja; 1 sampai 13 ms ujung ke ujung atas 12 run produksi tersimpan | Keduanya jalur dalam proses, **bukan** round-trip jaringan ke endpoint nyata. Angka lapangan **belum diukur** |
